...
int main(int argc, char *argv[]) {
    ...
    thrID = (int *)malloc(num_threads * sizeof(int));
...
// VS: ... parallel merge sort
    ...
    for (i = 0; i < num_threads; i++)
        pthread_create(&threads[i], NULL, sort_list, &thrID[i]);
...
// VS: ... destroy mutex, condition variables, etc.
    ...
    free(list); free(work); free(list_orig); free(thrID); 
