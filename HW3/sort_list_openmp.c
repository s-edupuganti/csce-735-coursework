...
void sort_list(int q) {
    ...
    // Sort local lists
    #pragma omp parallel for default(none) num_threads(num_threads) shared(list, ptr, num_threads) private(my_list_size)
        for (my_id = 0; my_id < num_threads; my_id++) {
    ...
    #pragma omp parallel for num_threads(num_threads) default(none) shared(ptr, np, level, list, num_threads, list_size, q, work) private(i, my_id)
        for (my_id = 0; my_id < num_threads; my_id++) {
    ...
    #pragma omp parallel for num_threads(num_threads) default(none) shared(num_threads, work, list, ptr) private(i, my_id)
        for (my_id = 0; my_id < num_threads; my_id++) {
